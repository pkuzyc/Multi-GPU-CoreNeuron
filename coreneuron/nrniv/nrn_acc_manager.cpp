...
void setup_nrnthreads_on_device(NrnThread* threads, int nthreads) {
     ...
     */
    printf("mpi id in acc_manager.cpp:%d\n",nrnmpi_myid);
    acc_set_device_num(nrnmpi_myid%num_gpus,acc_device_nvidia);
    ...
    for (i = 0; i < nthreads; i++) {
        ...
        //copy min_v to device
        double* d_min_v=(double*)acc_copyin(nt->_min_v,nt->end*sizeof(double));
        acc_memcpy_to_device(&(d_nt->_min_v),&(d_min_v),sizeof(double*));
